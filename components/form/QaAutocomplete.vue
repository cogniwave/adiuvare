<template>
  ...
  >
    ...
    <template v-slot:chip="{ item }">
      <v-chip removable @click:close="onRemoveChip(item.value)">
        {{ item.value }}
      </v-chip>
    </template>
  </v-autocomplete>
...
const props = defineProps({
  ...
  error: { type: [String, Array] as PropType<string | string[]>, default: "" },
...
const $emit = defineEmits<{
  ...
  (e: "click:remove-chip", val: string): void;
}>();
...
const onRemoveChip = (val: string) => $emit("click:remove-chip", val);
</script>
